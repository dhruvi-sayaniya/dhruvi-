# Importing ToolKits
import re
# Always use fallback stub for vizualizations if the module is not available.
try:
    import vizualizations
except ImportError:
    # Fallback stub for vizualizations when the real module is not available.
    # Provides the minimal functions used by the app to avoid import errors
    # and allow the app to run (renders simple placeholders).
    class _VizFallback:
        @staticmethod
        def create_vizualization(df, viz_type="box", data_type=None):
            # simple placeholder that writes a message to Streamlit
            import streamlit as st
            try:
                st.write(f"Visualization placeholder: {viz_type} (data_type={data_type})")
            except Exception:
                # If Streamlit is not available in this context, silently pass
                pass

        @staticmethod
        def create_heat_map(df):
            # return a minimal Plotly figure as a placeholder
            import plotly.express as px
            import numpy as np
            arr = np.zeros((2, 2))
            return px.imshow(arr, color_continuous_scale="Blues")

    vizualizations = _VizFallback()
